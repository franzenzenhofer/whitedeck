...
export const normalizeLinkTarget = (url: string): string => {
  try {
...
export const verifyKey = async (deck: Deck, keyPath: string): Promise<void> => {
  ...
  for (const uri of uris) {
    if (MUST_ENCODE_RE.test(uri)) defects.push(`link target carries a character that must be encoded: ${uri}`);
  }
  const addressed = new Set(uris.map(normalizeLinkTarget));
  for (const target of deckLinkTargets(deck)) {
    if (!addressed.has(normalizeLinkTarget(target))) defects.push(`link not clickable in the .key: ${target}`);
  }
  if (defects.length > 0) {
