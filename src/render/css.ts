...
import type { ThemePlaceholder } from '../theme/types.js';
...
type JsonPlaceholder = (typeof whiteTheme.layouts)['title-bullets']['placeholders'][number];

const box = (ph: JsonPlaceholder | ThemePlaceholder): string =>
  [
    '  position: absolute;',
    `  left: ${ph.xPx}px;`,
    `  top: ${ph.yPx}px;`,
...
const text = (ph: JsonPlaceholder | ThemePlaceholder): string =>
  [
...
const titleRule = (id: string, ph: JsonPlaceholder): string =>
  rule(
    `section.${id} h1`,
    `${box(ph)}\n${text(ph)}\n  display: flex;\n  flex-direction: column;\n  justify-content: center;`,
  );

const bulletBodyRule = (id: string, ph: JsonPlaceholder): string =>
  [
...
const subtitleRule = (id: string, ph: JsonPlaceholder): string =>
  rule(`section.${id} h2, section.${id} p`, `${box(ph)}\n${text(ph)}\n  font-weight: ${FONT_WEIGHTS[ph.font] ?? 400};`);

const quoteRules = (id: string, quotePh: JsonPlaceholder, attributionPh: JsonPlaceholder): string =>
  [
...
const picRules = (id: string, pics: readonly JsonPlaceholder[]): string =>
  pics
    .map((ph, index) =>
      rule(
...
const layoutRules = (id: string): string => {
  const layout = whiteTheme.layouts[id as keyof typeof whiteTheme.layouts];
  const placeholders = layout.placeholders as readonly JsonPlaceholder[];
  const rules: string[] = [`section.${id} {}`];

  const title = placeholders.find((p) => p.role === 'title');
  if (title) rules.push(titleRule(id, title));

  const bodies = placeholders.filter((p) => p.role === 'body');
  if (id === 'quote') {
  ...
  const pics = placeholders.filter((p) => p.role === 'pic');
...
export const themeCss = (): string => {
  const { canvas, background } = whiteTheme;
  const base = [
    '/* @theme keynote-white */',
    rule(
      'section',
  ...
  const layouts = Object.keys(whiteTheme.layouts).map((id) => layoutRules(id));
